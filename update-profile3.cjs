...
function replaceBlock(startStr, endStr, newBlock, blockName) {
  const startIdx = content.indexOf(startStr);
  if (startIdx === -1) {
    console.error(`Could not find start for ${blockName}`);
    return;
  }
  const endIdx = content.indexOf(endStr, startIdx);
  if (endIdx === -1) {
    console.error(`Could not find end for ${blockName}`);
    return;
  }
  
  content = content.substring(0, startIdx) + newBlock + content.substring(endIdx + endStr.length);
  console.log(`Successfully replaced ${blockName}`);
}

// 1. Container Style
const containerStart = `              className={\`relative w-full max-w-md bg-[#0b0c0e] rounded-2xl overflow-hidden shrink-0 transition-all border-2 \${`;
const containerEnd = `              }\`}`;
const newContainer = `              className={\`relative w-full max-w-md bg-[#0b0c0e] rounded-2xl overflow-hidden shrink-0 transition-all border-2 \${
                (() => {
...
replaceBlock(containerStart, containerEnd, newContainer, 'Container Style');

// 2. Avatar Border
const avatarStart = `                    <div className={\`w-20 h-20 rounded-full bg-zinc-950 flex items-center justify-center text-3xl font-black text-white shrink-0 overflow-hidden border-4 relative \${`;
const avatarEnd = `                    }\`}>`;
const newAvatar = `                    <div className={\`w-20 h-20 rounded-full bg-zinc-950 flex items-center justify-center text-3xl font-black text-white shrink-0 overflow-hidden border-4 relative \${
                      (() => {
                        const r = selectedUserProfile.role.toLowerCase();
...
replaceBlock(avatarStart, avatarEnd, newAvatar, 'Avatar Style');

// 3. Role Badge Overlay
const roleStart = `                    {/* Role badge overlay */}`;
const roleEnd = `                    </span>`;
const newRole = `                    {/* Role badge overlay */}
                    {(() => {
                      const isPro = selectedUserProfile.hasProAccess || selectedUserProfile.isYou;
                      const r = selectedUserProfile.role.toLowerCase();
...
replaceBlock(roleStart, roleEnd, newRole, 'Role Badge');

// 4. Linked Nodes
const linkedStart = `                    <div className="flex items-center justify-between mb-2">
...
const linkedEnd = `                      </h3>`;
const newLinked = `                    <div className="flex items-start justify-between mb-2">
...
replaceBlock(linkedStart, linkedEnd, newLinked, 'Linked Nodes');

// 5. Vibe Endorsements block
const vibeStart = `                  <div className="mt-4">
...
const newVibe = `                  <div className="mt-4">
                    {(() => {
                      const r = selectedUserProfile.role.toLowerCase();
...
// Wait, doing this replacement with indexOf can be dangerous if vibeEnd occurs multiple times inside the vibeBlock. Let's find the specific block index.
{
  const startIdx = content.indexOf(vibeStart);
  if (startIdx !== -1) {
    // Find the next occurrence of vibeEnd after startIdx
    const endIdx = content.indexOf(vibeEnd, startIdx);
    if (endIdx !== -1) {
      content = content.substring(0, startIdx) + newVibe + content.substring(endIdx + vibeEnd.length);
      console.log('Successfully replaced Vibe Endorsements');
    }
  }
}

// 6. Tabs Navigation
const tabsNavStart = `                {/* Profile Tabs Navigation */}`;
const tabsNavEnd = `                </div>`;
const newTabsNav = `                {/* Profile Tabs Navigation */}
                {(() => {
                  const r = selectedUserProfile.role.toLowerCase();
                  const isArtist = r.includes('artist') || r.includes('band');
...
{
  const startIdx = content.indexOf(tabsNavStart);
  if (startIdx !== -1) {
    const endIdx = content.indexOf(tabsNavEnd, startIdx);
    if (endIdx !== -1) {
      content = content.substring(0, startIdx) + newTabsNav + content.substring(endIdx + tabsNavEnd.length);
      console.log('Successfully replaced Tabs Navigation');
    }
  }
}
