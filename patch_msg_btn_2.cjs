...
code = code.replace(/const targetUser = selectedUserProfile;\s*setSelectedUserProfile\(null\);\s*setActiveTab\('messages'\);\s*setChats\(prev => \{[\s\S]*?return \[newC, \.\.\.prev\];\s*\}\s*\}\);\s*\}\);\s*triggerNotification\?\(`⚡ Opened encrypted channel with \$\{targetUser\?\.name \|\| 'Unknown Label'\}\`\);\s*\}\}/g,
